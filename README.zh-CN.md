...
安装脚本不会安装 Python、不会安装系统级 Tesseract，也不会自动创建 `.venv`。如果跳过可选的虚拟环境步骤，脚本会修改当前 `python` 指向的环境。它使用 editable installation，因此安装后不要删除或移动源码目录；如需移动，请重新运行安装脚本。
